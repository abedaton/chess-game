...
SuperGame::SuperGame(int gameMod, AbstractPlayer* client, bool myTurn, Player* play_one, Player* play_two): _client(client){
    ...
    switch(chessMod){
        case 0:
            game = new ClassicChess(lowPlayer, highPlayer, first, dico, aff);
            break;
        case 1:
            game = new DarkChess(lowPlayer, highPlayer, first, play_one, dico, aff);
            break;
        case 2:
            game = new TrappistChess(lowPlayer, highPlayer, first, dico, aff);
            break;
        case 3:
            game = new AntiChess(lowPlayer, highPlayer, first, dico, aff);
            break;
        default:
            game = new ClassicChess(lowPlayer, highPlayer, first, dico, aff);
            break;
...
int SuperGame::opponentMov(std::string mov){
    std::cout << "opponentMov: " << mov << ", " << this->_player2 << ", " << this->_inverted << std::endl;
    int result = 0;
    std::pair<bool, bool> res = this->_game->execute_step(mov, this->_player2, false);
    std::cout << "answer from game: " << res.first << std::endl;
    this->_ListMov.clear();
	if (res.second)
		result = -1;
		...
		mov = this->_bufferMov.front();
		this->_bufferMov.erase(_bufferMov.begin());
        res = this->_game->execute_step(mov, this->_player1, false);
		if (res.first) {
            this->_client->mov(mov);
            if(res.second) {
                result = 1;
