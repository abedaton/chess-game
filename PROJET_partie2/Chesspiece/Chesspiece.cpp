...
Chesspiece::Chesspiece() : limite(0), position(nullptr), liste_depl(nullptr), liste_capt(nullptr), depl(nullptr), owner(new Human("")), capt(nullptr),updt_needed(false) {
	this->depl = new Decoder(this);
	this->capt = new Decoder(this);
	} //*< Constructor
	
Chesspiece::Chesspiece(const Chesspiece& pe) : limite(pe.limite), position(pe.position), liste_depl(pe.liste_depl), liste_capt(pe.liste_capt), depl(pe.depl), owner(pe.owner), capt(pe.capt),updt_needed(pe.updt_needed) {}
